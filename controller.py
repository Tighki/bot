...
class DroneController(Node):
    ...
    def fly(self):
        ...
        if not self.current_state.armed:
            self.arm_drone()
            return  # Ждем, пока дрон будет вооружен
        
        # Взлет
        if not self.is_flying:
            self.flight_manager.take_off(self.pose, self.altitude)
            self.is_flying = True  # Устанавливаем флаг в true после успешного взлета
        else:
            # Если дрон уже летит, проверяем необходимость посадки
            if self.landing:
                self.land.take_off(self.pose, self.altitude)
